...
switch(lower(option))
    case 'init'
        ...
        diva_vtdisp_Ricky(data.handles.hfig,'play',[zeros(10,1);0;1;1]);
        drawnow;
    ...
    case 'setslider' % called when a slider is moved, mouse is released  
        ...
        if barData.ready2play
            diva_vtdisp_Ricky(hfig,'play',x);
        else
            diva_vtdisp_Ricky(hfig,'update',x);
        end
    ...
    case 'play'
        ...
        for n=[1:9:size(data.state.x,2)-1,size(data.state.x,2)]
            ...
            [data.state.Aud,data.state.Som,data.state.Outline,data.state.af,data.state.filt]=diva_synth(data.state.x(:,n),'explicit'); % this in particular
            ...
            %the area and frequency function here are the important ones^
        end
        ...
        if isfield(data, 'mouseIsDown')
            if data.mouseIsDown
                ...
                if data.curBar <= barLim && data.curBar > 0.5 && pos2(1,2) >= -3 && pos2(1,2) <= 3
                    ...
                    diva_vtdisp_Ricky(data.handles.hfig,'setslider',data);
                    % old
                    %diva_vtdisp_Ricky(data.handles.hfig,'update',data);
                    drawnow;
                end
            end
        end
