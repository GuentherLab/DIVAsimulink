...
switch(lower(option))
    ...
    case 'setslider' % called when a slider is moved, or mouse is released
        ...
        if isfield(stateData, 'curBar') && isfield(stateData, 'curBarVal')
            n=stateData.curBar;
            v=stateData.curBarVal;
        elseif isfield(stateData, 'newVocalT')
            v=0;
        else
            return;
        end
        ...
        if isfield(stateData, 'newVocalT')
            x=diva_solveinv(x,stateData.newVocalT,'outline','lambda',0.02,'center',stateData.oldVocalT);
            x=max(-1,min(1,x));
            %diva_vtdisp_Ricky(hfig,'test',x,stateData);
            diva_vtdisp_Ricky(hfig,'updsliders',x,stateData);
            stateData = rmfield(stateData,'newVocalT'); % remove vCord var so that GUI doesn't always assume you are changing the vocal cord
        end
        x=repmat(x,[1,100]);
        x(n,:)=[linspace(x(n,1),v,20),repmat(v,1,80)];
        diva_vtdisp_Ricky(hfig,'update',x,stateData);
    ...
    case 'update'
        ...
        if exist('stateData', 'var') == 1 && isfield(stateData, 'curBar')
            if stateData.curBar == 13
                if stateData.curBarVal >= 0.6
                    set(data.handles.h1a, 'visible', 'on');
                    set(data.handles.h1b, 'visible', 'on');
                    set(data.handles.h1c, 'visible', 'on');
                elseif stateData.curBarVal >= 0.3
                    set(data.handles.h1a, 'visible', 'on');
                    set(data.handles.h1b, 'visible', 'on');
                    set(data.handles.h1c, 'visible', 'off');
                elseif stateData.curBarVal >= 0
                    set(data.handles.h1a, 'visible', 'on');
        ...
        if exist('stateData', 'var') == 1
            if stateData.ready2play
    ...
    function downcallback(varargin)
        ...
        % Need to figure out what plot was clicked, so check from right to left:
        % idea here is that we look at pos2 (which relates to the articulator axis) first, if
        % its x value is greater than 1, then we assume the other plot was picked instead
        if mArtPos(1) > 1
