...
DIVA_x.debug=0;
  ...
  if ~nargin,
      if ~DIVA_x.debug
          [Ngl,glname]=diva_glottalsystem_forwardmodel;
          switch(glname) % note: to add a new glottal model, add a new 'case' below detailing the motor labels and ranges of that model
              case 'LF'
                  assert(Ngl==3);
                  Nart=13;
                  Input=struct(...
                      ...
                      'DefaultSilence',[repmat([-1,1],[10,1]);-1,1;-1,-.5;-1,1],... % note: manually define gl ranges for silence/sound
              ...
              otherwise
                  error('unrecognized glottal model %s',glname);
          end
          Output1=struct(...
              'Name','Auditory',...
              'Dimensions',4,...
